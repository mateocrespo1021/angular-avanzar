...
export class PublicacionInventoryDestacadosService {
  ...
  listarServicio(): void {
    this._httpClient.get<InventarioPublicaciones[]>(environment.baseUrlm+`/api/publicaciones/listarDestacados/${this.id}`)
      .subscribe((data) => {
  ...
  obtenerListaPublicaciones(): Observable<InventarioPublicaciones[]> {
    return this._httpClient.get<InventarioPublicaciones[]>(environment.baseUrlm+`/api/publicaciones/listarDestacados/${this.id}`)
      .pipe(
